...
char* __not_in_flash_func(fast_itoa)(int value, char* buffer_end)
{
    ...
    *ptr = '\0'; // Set the null terminator
    ...
    // We use an unsigned int to safely handle INT_MIN (-2147483648) 
    // without causing an undefined behavior overflow when negating
...
void __not_in_flash_func(core1_entry)()
{
    ...
    serial_program_init(serial_pio, serial_sm, serial_offset, 26, 115200);

    gpio_put(PICO_DEFAULT_LED_PIN, 1);
    busy_wait_ms(500);

    serial_program_puts(serial_pio, serial_sm, "\033[2JCore1 serial ready\r\n");

    volatile uint8_t *light = &sram[0x1c00];    // light points ta memory location voliatile makes sure it passes between cores
    while (true)
    {
        uint8_t serial_byte = *light;
        gpio_put(PICO_DEFAULT_LED_PIN, serial_byte <= 127);

        char serial_text[6];
        char *serial_number = fast_itoa(serial_byte, &serial_text[3]);
        serial_text[3] = '\r';
        serial_text[4] = '\n';
        serial_text[5] = '\0';
